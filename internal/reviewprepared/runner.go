...
const DefaultPRBranch = "planwerk-agent/improve-prepared-features"
...
func (r *Runner) openPR(repo *github.Repo, result *Result, opts Options) (string, error) {
	...
	commit := title + "\n\nGenerated by planwerk-agent review-prepared."
...
// The attribution footer follows the house linked-footer style (matching audit
// and gap-analysis): a link back to planwerk-agent and the model that produced
// the rewrite, so the PR names its origin like every other generated artifact.
func buildPRBody(result *Result) string {
	...
	fmt.Fprintf(&sb, "_Generated by %s review-prepared %s_\n\n", attribution.Tool(), attribution.AssistantWith(model))
	if result == nil {
		return sb.String()
	}
	if result.Overview != "" {
