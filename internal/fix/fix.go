...
// leave on GitHub.
func fixCommentFooter(model string) string {
	return "_Fix report generated by " + attribution.Tool() + " fix " + attribution.AssistantWith(model) + "_"
}
...
// formatFixComment wraps the iteration's report in the PR-comment body: the
// report verbatim (it already carries its own "## Fix Report (iteration N)"
