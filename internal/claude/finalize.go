...
// commits and links the issue with "Closes #N". It is the final step of an
// implement run — the implement, simplify, and review sessions deliberately do
// NOT push or open a PR, so the PR lands already simplified and self-reviewed.
//
...
// are handled in the session), push the branch, and open the draft PR with the
// mandatory closing-keyword link to the issue. Exported so the implement path can
// render the prompt without invoking Claude.
