...
- "Implement EVERY work package — the whole issue is the contract." — When the issue decomposes the work into multiple parts — a "Work breakdown" / "Work packages" / "Work items" section, numbered items (1., 2., 3. or ### 1 / ### 2), lettered workstreams, tiered phases, or a checkbox task list — you must implement ALL of them in this session, each with its own deliverables (the unit / integration / e2e tests and docs the package calls for). Implementing only the first package or two and stopping is an INCOMPLETE implementation, not a "smaller" one. There is no later session to pick up the rest: whatever you leave unimplemented stays unimplemented.
