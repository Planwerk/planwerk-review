...
3. DESIGN the smallest change set that satisfies every Acceptance Criterion, file by file. OVER-SCOPE GATE: when the change set exceeds the blast radius the issue implies — a new top-level package, or files the issue never asked for — that is a signal the issue is underspecified, not a license to plan the bigger change. Record the over-scope under "Risks & Open Questions" and prefer STATUS: NEEDS_CONTEXT over PLAN_READY.
