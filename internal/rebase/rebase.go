...
func analysisCommentFooter(model string) string {
	return "_Rebase analysis generated by " + attribution.Tool() + " rebase " + attribution.AssistantWith(model) + "_"
}
...
func (r *Runner) postAnalysisComment(w io.Writer, opts Options, owner, repo string, number int, body, model string) {
	if opts.NoAnalysisComment {
		return
	}
	comment := strings.TrimSpace(body) + "\n\n---\n\n" + analysisCommentFooter(model) + "\n"
